...
#### 目录
- [同源策略](#二-同源策略-same-origin-policy-sop-详解)
- [跨源资源共享](#三-跨源资源共享-cors-详解)
- [CORS 请求类型与流程](#四-cors-请求类型与流程-request-types-and-flow)
- [深入理解 `Access-Control-Allow-Origin`](#五-深入理解-access-control-allow-origin-acao)
- [CORS 配置错误与安全风险](#六-cors-配置错误与安全风险-misconfigurations-and-security-risks)
- [其他跨域技术](#七-其他跨域技术-other-cross-origin-techniques)
...
CORS 配置错误本身不直接导致代码执行，但它可以被其他漏洞利用，放大危害。
